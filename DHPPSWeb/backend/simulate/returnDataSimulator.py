...
class CLSTM_cell(nn.Module):
    def __init__(self, shape, input_chans, filter_size, num_features):
        ...
        self.conv = nn.Conv2d(self.input_chans + self.num_features,
    ...
    def init_hidden(self, batch_size):
        return (Variable(
            torch.zeros(batch_size, self.num_features, self.shape[0],
                        self.shape[1])),
                Variable(
                    torch.zeros(batch_size, self.num_features, self.shape[0],
                                self.shape[1])))
...
class CLSTM(nn.Module):
    def __init__(self, shape, input_chans, filter_size, num_features,
        ...
        cell_list = []
        cell_list.append(
            CLSTM_cell(self.shape, self.input_chans, self.filter_size,
                       self.num_features))
        for idcell in range(1, self.num_layers):
            cell_list.append(
                CLSTM_cell(self.shape, self.num_features, self.filter_size,
                           self.num_features))
...
def AcquireAllData(popuList, transMatrix, infectedList):
    ...
    for i in range(length):
        returnList.append(
            AcquireData(popuList[i], transMatrix[i], infectedList[i]))
    return returnList
